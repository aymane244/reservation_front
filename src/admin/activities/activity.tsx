...
export default function AdminActivities(){
    ...
    const addActivity = async () => {
        ...
        try {
            const res = await axiosInstance.post('api/activities', { name: newName });
            if(res.status === 200){
    ...
    const updateActivity = async (id: number) => {
        ...
        try {
            const res = await axiosInstance.post(`api/activities/${id}`, { name: editingName });
            if(res.status === 200){
                setActivities((prev) =>
    ...
    const toggleStatus = async (id: number, currentStatus: number) => {
        try{
            const res = await axiosInstance.post(`api/activities-status/${id}`, {
    ...
    const deleteActivity = async (id: number) => {
        try{
            ...
            if(result.isConfirmed){
                const res = await axiosInstance.post(`api/activities-delete/${id}`);
    ...
    const addPlanToActivity = async () => {
        ...
        try{
            const res = await axiosInstance.post('api/activities/add-plan', {
                name: planName,
                price: planPrice,
                                    ...
                                    <td>
                                        {editingId === activity.id ? (
                                            ...
                                            <>
                                                ...
                                                <FontAwesomeIcon
                                                    icon={faTrashAlt}
                                                    className="text-danger fs-5 mx-4 pointer"
                                                    onClick={() => deleteActivity(activity.id)}
                                                />
                                                <FontAwesomeIcon
                                                    icon={faPlus}
                                                    className="text-primary fs-5 pointer"
